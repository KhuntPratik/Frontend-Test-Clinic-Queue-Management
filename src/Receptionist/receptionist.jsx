...
export default function ReceptionistHome() {
  ...
  const handleDone = async (queueId) => {
    ...
    try {
      ...
      setQueue((prev) =>
        prev.map((item) =>
          (item.id || item._id) === queueId ? { ...item, status: 'done' } : item
        )
      )
    } catch (err) {
      const data = err?.response?.data
      ...
      <div className="container py-4">
        ...
        <div className="card">
          <div className="card-body">
                        ...
                        <td>
                          <div className="d-flex gap-2">
                            ...
                            'waiting' ? (
                              ...
                              <button
                                className="btn btn-outline-secondary btn-sm"
                                onClick={() => handleDone(item.id || item._id)}
                                disabled={
                              ...
                              >
                                {(item.status || item.queueEntry?.status) === 'done'
                                  ? 'Done'
                                  : 'Mark done'}
                              </button>
                            )}
           ...
           Loading queue...' : 'No queue available.
              </p>
            )}
          </div>
        </div>
